#!/usr/bin/env python3
"""
Script to add OLD: and NEW: comment prefixes to the replaced function calls.
"""

import re

def add_comment_prefixes(file_path):
    """Add OLD: and NEW: comment prefixes to history function calls."""
    ...
    while i < len(lines):
        ...
        if 'self.game.add_history_entry_unified(' in line and not line.strip().startswith('#'):
            ...
            # Add OLD: comment for this line
            old_line = line.replace('add_history_entry_unified(', 'add_history_entry_with_retry(')
            old_line = old_line.replace('self.player_index,', 'self.player_index + 1,')
            ...
            modified_lines.append(f"{indent_str}# NEW: Use unified history function with 0-based player index\n")
...
def main():
    ...
    try:
        new_content = add_comment_prefixes(file_path)
